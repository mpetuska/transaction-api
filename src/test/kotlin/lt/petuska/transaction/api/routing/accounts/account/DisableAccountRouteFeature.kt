...
@KtorExperimentalAPI
object DisableAccountRouteFeature : Spek({
  Feature("Disable account") {
    Scenario("No account exist") {
      ...
      When("Activating an account") {
        call = testEngine.handleRequest(HttpMethod.Put, "/v1/accounts/5/disable")
      }
    ...
    Scenario("An account exists") {
      ...
      Given("A user has active accounts") {
        testAccount = testEngine.createTestAccount(testUser.id, "Active Acc", Currency.EUR, true)
        testAccount2 = testEngine.createTestAccount(testUser.id)
      }

      When("Disabling an account") {
        call = testEngine.handleRequest(HttpMethod.Put, "/v1/accounts/${testAccount.id}/disable")
      }
